...
- Now, create and select a virtual environment in VSCode. If the GUI doesn't work then do this in the terminal by rinning `python -m venv .venv`.
...
# Does not work due to a dash '−' instead of a minus '-' in one of the rows.
# emissions_long["emissions"] = pd.to_numeric(emissions_long["emissions"])

# Solution:

# Option 1: Coerce that value to a NaN:
# emissions_long["emissions"] = pd.to_numeric(emissions_long["emissions"], errors="coerce")

# Option 2: Replace the dash with a minus, then convert to numeric.
# To do this, first force the values to be recognised as a string using `astype(str)`, then replace using `str.replace()`.
emissions_long["emissions"] = (
    emissions_long["emissions"].astype(str).str.replace("−", "-")
)
# emissions_long['emissions'].str.replace('−', '-', regex=False)
emissions_long["emissions"] = pd.to_numeric(emissions_long["emissions"])
...
countries_of_interest = emissions_long_1990_2022.query(
    "country.isin(['Egypt', 'Nigeria', 'India', 'Sweden', 'UK'])"
)
